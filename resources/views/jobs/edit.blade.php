        ...
        <div class="mx-4">
            <x-card class="bg-gray-50 border border-gray-200 p-10 rounded max-w-lg mx-auto mt-24">
                <header class="text-center">
                    ...
                    <p class="mb-4">Edit {{ $job->job_title }}</p>
                </header>
                ...
                <form action="{{ route('update', $job->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" class="border border-gray-200 rounded p-2 w-full" value="{{ $job['company'] }}" name="company">
                        @error('company')
                            <p class="text-red-500  text mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" class="border border-gray-200 rounded p-2 w-full" name="job_title"
                            placeholder="Example: Senior Laravel Developer" value="{{ $job['job_title'] }}" { />
                        @error('job_title')
                            <p class="text-red-500  text mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" class="border border-gray-200 rounded p-2 w-full" name="company_site">
                        @error('company_site')
                            <p class="text-red-500  text mt-1">{{ $message }}</p>
                        @enderror
                    </div>
